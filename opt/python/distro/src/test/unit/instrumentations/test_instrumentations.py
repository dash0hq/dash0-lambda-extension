...
def test_instrumentation_disabled_via_env_var_single(monkeypatch):
    """Test that a single instrumentation can be disabled via DASH0_DISABLE_INSTRUMENTATION"""
...
def test_instrumentation_disabled_via_env_var_multiple(monkeypatch):
    """Test that multiple instrumentations can be disabled via DASH0_DISABLE_INSTRUMENTATION"""
    monkeypatch.setenv("DASH0_DISABLE_INSTRUMENTATION", "boto,langchain")
...
def test_instrumentation_not_disabled_when_not_in_list(monkeypatch):
    ...
    monkeypatch.setenv("DASH0_DISABLE_INSTRUMENTATION", "langchain")
    fake_boto = MagicMock()
    with patch.dict("sys.modules", {"boto": fake_boto}):
...
def test_instrumentation_enabled_when_env_var_empty(monkeypatch):
    """Test that all instrumentations work when DASH0_DISABLE_INSTRUMENTATION is empty"""
...
def test_instrumentation_disabled_takes_precedence_over_lambda_check(monkeypatch):
    """Test that DASH0_DISABLE_INSTRUMENTATION takes precedence over Lambda environment checks"""
    monkeypatch.setenv("DASH0_DISABLE_INSTRUMENTATION", "langchain")
    fake_langchain = MagicMock()
    with patch.dict("sys.modules", {"langchain": fake_langchain}):
...
def test_instrumentation_disabled_respects_case_sensitivity(monkeypatch):
    ...
    monkeypatch.setenv("DASH0_DISABLE_INSTRUMENTATION", "Boto")  # Capital B
    fake_boto = MagicMock()
    with patch.dict("sys.modules", {"boto": fake_boto}):
