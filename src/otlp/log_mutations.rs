...
/// Unrecognized severity text maps to UNSPECIFIED (0) rather than guessing a level.
...
pub fn map_logs_to_otlp(logs: &[TelemetryLog]) -> Vec<LogRecord> {
    ...
    for log in logs {
        ...
        // dash0_payload logs (lambda_event, lambda_return_value, http bodies, ...) are
        // synthetic diagnostic dumps generated by the extension itself, not user log
        // lines, so - like platform logs - INFO is accurate rather than "unparsed".
        let is_dash0_payload_log = log
            .custom_attributes
            .iter()
            .any(|(key, _)| key == crate::otlp::attributes::DASH0_FAAS_PAYLOAD_TYPE);
        ...
        let (severity_number, severity_text) = match &severity {
            Some(sev) => (severity_text_to_number(sev), sev.to_uppercase()),
            None => (0, String::new()),
        };
...
mod tests {
    ...
    fn test_platform_logs_have_info_severity() {
        ...
        // Verify function log has unset severity when no severity pattern found
        assert_eq!(result[1].severity_number, 0); // UNSPECIFIED
    ...
    fn test_severity_text_to_number_mapping() {
        ...
        // Unrecognized severity text maps to UNSPECIFIED
        assert_eq!(severity_text_to_number("UNKNOWN"), 0);
    }
    ...
    #[test]
    fn test_function_log_without_node_pattern_leaves_severity_unset() {
        let logs = vec![TelemetryLog {
            time: "2026-03-02T11:53:38.040Z".to_string(),
            r#type: "function".to_string(),
        ...
        let result = map_logs_to_otlp(&logs);
        assert_eq!(result.len(), 1);
        assert_eq!(result[0].severity_number, 0); // UNSPECIFIED
        assert_eq!(result[0].severity_text, "");
    ...
    fn test_node_severity_levels_table() {
        let uuid = "f0ae1bc7-ae4b-4317-abf9-3a562e3127ef";
        let ts = "2026-03-02T11:53:38.040Z";
        for (level, expected_number) in KNOWN_LEVELS {
            let body = format!("{ts}\t{uuid}\t{level}\tmessage body");
            let (num, text) = severity_for_body(&body);
            assert_eq!(num, *expected_number, "level {level}");
            assert_eq!(text, *level, "level {level}");
        }
    }

    #[test]
    fn test_python_severity_levels_table() {
        let uuid = "f0ae1bc7-ae4b-4317-abf9-3a562e3127ef";
        let ts = "2026-03-02T11:53:38.040Z";
        for (level, expected_number) in KNOWN_LEVELS {
            let body = format!("[{level}]\t{ts}\t{uuid}\tmessage body");
            let (num, text) = severity_for_body(&body);
            assert_eq!(num, *expected_number, "level {level}");
            assert_eq!(text, *level, "level {level}");
        }
    ...
    #[test]
    fn test_unrecognized_severity_token_maps_to_unspecified_number() {
        // The Node/Python prefix patterns will happily capture a token in the
        // severity slot even if it isn't one of the canonical OTel levels (e.g. a
        // custom log-level name). severity_text should reflect what was parsed, but
        // severity_number must not guess a level for text it doesn't recognize.
        let uuid = "f0ae1bc7-ae4b-4317-abf9-3a562e3127ef";
        let ts = "2026-03-02T11:53:38.040Z";
        let body = format!("{ts}\t{uuid}\tNOTICE\tsomething happened");
        let (num, text) = severity_for_body(&body);
        assert_eq!(num, 0); // UNSPECIFIED
        assert_eq!(text, "NOTICE");
    }

    #[test]
    fn test_arbitrary_unparseable_payloads_leave_severity_unset() {
        let uuid = "f0ae1bc7-ae4b-4317-abf9-3a562e3127ef";
        let ts = "2026-03-02T11:53:38.040Z";
        let cases: Vec<String> = vec![
            String::new(),
            "just a plain message with no structure".to_string(),
            r#"{"jsonLog": true, "value": "parsed"}"#.to_string(),
            format!("{ts}\t{uuid}\tmessage missing the severity field"), // only 3 fields
            format!("not-a-timestamp\t{uuid}\tINFO\tmessage"),
            format!("{ts}\tnot-a-uuid\tINFO\tmessage"),
            format!("[INFO]\tnot-a-timestamp\t{uuid}\tmessage"),
            format!("[INFO]\t{ts}\tnot-a-uuid\tmessage"),
            "🚀 unicode log message with no structure 日本語".to_string(),
            "a".repeat(200_000), // large unstructured payload
        ];
        for body in cases {
            let (num, text) = severity_for_body(&body);
            let preview: String = body.chars().take(60).collect();
            assert_eq!(num, 0, "body: {preview}");
            assert_eq!(text, "", "body: {preview}");
        }
    }

    #[test]
    fn test_severity_parsing_at_various_payload_sizes() {
        let uuid = "f0ae1bc7-ae4b-4317-abf9-3a562e3127ef";
        let ts = "2026-03-02T11:53:38.040Z";
        for size in [0usize, 1, 100, 10_000, 500_000] {
            let message = "x".repeat(size);
            let body = format!("{ts}\t{uuid}\tERROR\t{message}");
            let (num, text) = severity_for_body(&body);
            assert_eq!(num, 17, "size {size}");
            assert_eq!(text, "ERROR", "size {size}");
        }
    }
}
