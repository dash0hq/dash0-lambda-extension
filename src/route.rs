...
pub async fn passthru_proxy(req: Request<Body>) -> Result<Response<Body>, Error> {
    ...
    println!("[LRAP] passthru_proxy - Method: {}, URI: {}", req.method(), req.uri());
    ...
    if !body_bytes.is_empty() {
        println!("[LRAP] passthru_proxy - Body: {}", String::from_utf8_lossy(&body_bytes));
    }
...
async fn validate_and_mangle_next_event(
    ...
    println!("[LRAP] aws request id: {}", _aws_request_id);
    println!("[LRAP] event payload: {}", String::from_utf8_lossy(&body_bytes));
